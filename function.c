...
/*void turnofgame(){						//flip이 되지 않아서 자동으로 다음 turn으로 넘어간다. 
	...
	while(flip_happens(ch_i, ch_j) == 1){
		...
		for(ch_i=0;ch_i<N;ch_i++){
			for(ch_j=0; ch_j<N;ch_j++){
				if(gameboard[ch_i][ch_j] == ' '){
					...
					//	break;												//하나라도 플립이 가능하다면 함수를 빠져나간다. 
					//}
		...
		for(ch_i=0;ch_i<N;ch_i++){
			for(ch_j=0; ch_j<N;ch_j++){
				if(gameboard[ch_i][ch_j] == ' '){
					...
					if(flip_happens(ch_i, ch_j) == 1){
																				//근데 모든 곳에서 플립이 안 일어나야 하는데? 그건 어카냐고,,,,,>하나라도가능하면나가면됨. 
						gameboard[ch_i][ch_j] = ' ';						//원래상태로 되돌려준다. 
						break;												//하나라도 플립이 가능하다면 함수를 빠져나간다. 
					}
					else {				
...
//		return 1; 
//	else
	else*/ if((turn == 0 || flip_happens(i, j) == 0) && (turn == 1 || flip_happens(i, j) == 0))		//양쪽모두뒤집기가불가능할때 
			return 1;
	else
		return 0; 
}
